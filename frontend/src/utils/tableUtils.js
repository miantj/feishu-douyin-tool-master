...
const getRecordFields = async (infoData, mappedFields) => {
    ...
    for (let field in mappedFields) {
        ...
        if (fieldConfig && fieldConfig.type === FieldType.Attachment) {
            ...
            console.log("getRecordFields ====>", feildId, key, infoData[key]);
            value = infoData[key]
        }
        ...
        // console.log("getRecordFields ====>", feildId, key, fieldConfig, value);
...
export async function getCellValueByCell(table, recordId, fieldId) {
    ...
    if (typeof cellValue === 'object' && cellValue !== null && Array.isArray(cellValue) && cellValue.length > 0)
