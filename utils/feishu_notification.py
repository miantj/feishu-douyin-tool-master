...
class FeishuNotifier:
    ...
    async def send_rich_text_message(self, title: str, content: str, 
    ...
    async def send_card_message(self, video_info: Dict[str, Any]) -> bool:
        ...
        if '/flow/api/trigger-webhook/' in self.webhook_url:
            ...
            text_content = self._format_flow_message(
                author_name, desc, digg_count, comment_count, share_count, video_url, current_time
            )
                ...
                "card": {
                    ...
                    "elements": [
                        ...
                        # 数据统计区块
                        {
                            "tag": "div",
                            "text": {
                                "content": self._format_statistics(digg_count, comment_count, share_count),
                                "tag": "lark_md"
                            }
                        },
                        # 时间信息
    ...
    def _format_flow_message(self, author_name: str, desc: str, digg_count: int, 
                           comment_count: int, share_count: int, video_url: str, current_time: str) -> str:
        """
        格式化流程webhook的消息内容
        """
        # 计算热度等级
...
🕐 检测时间：{current_time}"""
    ...
    def _format_statistics(self, digg_count: int, comment_count: int, share_count: int) -> str:
        """
        格式化统计数据
        """
...
🔥 **热度值：** {heat_score:,}"""
    ...
    def _calculate_heat_score(self, digg_count: int, comment_count: int, share_count: int) -> int:
        """
        计算综合热度值
        """
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        if (result.get('code') == 0 or 
                            result.get('msg') == 'success' or
                            'success' in str(result).lower()):
                            logger.info(f"飞书消息发送成功: {result}")
                            return True
                    
                    logger.error(f"飞书消息发送失败: status={response.status}, result={result}")
                    return False
